                   ...
                   struct fuse_file_info *fi)
{
    ...
    struct stat st;
    yfs_client::inum inum = ino; // req->in.h.nodeid;   fuse_ino_t == unsigned long    inum == long long
    yfs_client::status ret;
     ...
     mode_t mode)
{
  ...
  yfs_client::status ret = yfs_client::OK;
  yfs_client::inum inum = 0;
  ret = yfs->create(parent, name, inum, false);
  if (ret == yfs_client::OK) {
    e.ino = inum;
    getattr(e.ino, e.attr);
  } else {
...
main(int argc, char *argv[])
{
  ...
  mountpoint = argv[1];  // 挂载目录

  srandom(getpid());

  myid = random();

  yfs = new yfs_client(argv[2], argv[3]);  // argv[2]为extent_server的端口   argv[3]为lock_server的端口

  fuseserver_oper.getattr    = fuseserver_getattr;  // lab2_ready
  fuseserver_oper.statfs     = fuseserver_statfs;   // lab2_ready
  fuseserver_oper.readdir    = fuseserver_readdir;  // lab2_1
  fuseserver_oper.lookup     = fuseserver_lookup;   // lab2_1
  fuseserver_oper.create     = fuseserver_create;   // lab2_1
  fuseserver_oper.mknod      = fuseserver_mknod;    // lab2_1
  fuseserver_oper.open       = fuseserver_open;     // lab2_ready
  fuseserver_oper.read       = fuseserver_read;     // lab2_2
  fuseserver_oper.write      = fuseserver_write;    // lab2_2
  fuseserver_oper.setattr    = fuseserver_setattr;  // lab2_2
  fuseserver_oper.unlink     = fuseserver_unlink;   // lab3_1
  fuseserver_oper.mkdir      = fuseserver_mkdir;    // lab3_1
