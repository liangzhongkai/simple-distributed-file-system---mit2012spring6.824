...
private:
		...
		pthread_mutex_t chan_m_; // protect incref/decref to chan_
...
public:
		...
		int call1(unsigned int proc, // 连接服务端, 并接受返回进行初始化某些信息
...
private:
	...
	int port_;
	unsigned int nonce_;//server xid
	...
	// latest connection to the client    key:xid
	std::map<unsigned int, connection *> conns_;

	// counting rpc called times
	const int counting_;
	int curr_counts_;
...
public:
	...
	template<class S, class A1, class R>
		void reg(unsigned int proc, S*, int (S::*meth)(const A1, R & r));
